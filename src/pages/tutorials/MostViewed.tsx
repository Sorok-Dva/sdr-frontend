...
const MostViewed: React.FC = () => {
  ...
  useEffect(() => {
    const fetchTutorials = async () => {
      try {
        const response = await fetch('/api/tutorials?type=mostViewed')
          ...
          <div className="row">
            { currentTutorials.length === 0 ? (
              ...
              currentTutorials.map((tutorial) => (
                <div key={ tutorial.id } className="col-lg-4 col-md-6">
                  <div className="single-news">
                    <div className="blog-img">
                      <Link
                        to={ `/tutorial/${ tutorial.id }/${ slugify(tutorial.title) }` }>
                        <Image
                          src={ tutorial.image ?? defaultTutoImg }
                          alt="Image"
                          width={ 570 }
                          height={ 400 }
                          loader={ <ImageLoader height="200px"
                            width="360px"/> }
                          loading="lazy"
                        />
                      </Link>
                      ...
                      <ul>
                        <li>
                          <Link to={ tutorial.user?.nickname ? (`/user/${tutorial.user?.nickname}`) : ('#') }>
                            <i
                              className="flaticon-user"></i> { tutorial.user?.nickname ?? 'Unknown' }
                          </Link>
                        </li>
                        <li>
                          <i
                            className="fa fa-eye"></i> { tutorial.views } vues
                        </li>
                        <li>
                          <i
                            className="flaticon-conversation"></i> { tutorial.commentCount } Comments
                        </li>
                      </ul>
                      <Link
                        to={ `/tutorial/${ tutorial.id }/${ slugify(tutorial.title) }` }>

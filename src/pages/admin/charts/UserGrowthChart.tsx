...
const fetchUserData = async (timeframe : string) => {
  const token = localStorage.getItem('token')
  const response = await fetch(`/api/admin/charts/users?timeframe=${ timeframe }`, {
    method: 'GET',
    headers: {
...
const UserGrowthChart : React.FC = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      const data = await fetchUserData(timeframe)
      const labels = data.map((item : any) => item.date) // Extract dates for the x-axis
      const userCounts = data.map((item : any) => item.count) // Extract user counts
        ...
        ticks: {
          ...
          callback: function (value : string | number) : string {
            if (typeof value === 'number') {
              return `${ value }` // Format number tick as a string
            }
  ...
  return (
    <Container fluid>
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h2 className="text-white">User Growth</h2>
        <ButtonGroup>
